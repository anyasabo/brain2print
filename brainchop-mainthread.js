...
async function generateBrainMask(
  ...
  if (opts.isBrainCropMaskBased) {
    ...
    //  Brain tissue
    window.alert(
      "getAllSlicesData1D() is not dead code? niftiHeader and niftiImage required by getAllSlicesData1D",
    );
  }
...
class SequentialConvLayer {
  ...
  async apply(inputTensor) {
    ...
    return new Promise((resolve) => {
      ...
      const seqTimer = window.setInterval(async () => {
        ...
        this.callbackUI(`Iteration ${chIdx}`, chIdx / this.outChannels);
...
async function inferenceFullVolumePhase1(
  ...
  if (modelEntry.preModelId) {
    ...
    try {
      ...
      let maxLabelPredicted = 0;
      const delay =
        inferenceModelsList[modelEntry.preModelId - 1].inferenceDelay;
